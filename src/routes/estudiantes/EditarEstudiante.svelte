<script lang="ts">
	...
	import { estudiantesService, type EstudianteResponse, type EstudianteUpdateDTO } from '$lib/services/estudiantes';
	...
	let { estudiante, onClose, onSuccess }: Props = $props();

	let formData: EstudianteUpdateDTO = {
	...
	async function handleSubmit(e: Event) {
		...
		try {
			await estudiantesService.updateEstudiante(estudiante.id, formData);
			onSuccess();
		} catch (error: any) {
			errorMessage = error.message || 'Error al actualizar el estudiante';
		} finally {
			isSubmitting = false;
		}
	}
...
<div class="modal-overlay" onclick={handleOverlayClick}>
	<div class="modal-content">
		<div class="modal-header">
			<h2>{@html getIconSvg('edit')} Editar Estudiante</h2>
